...
impl Client {
    ...
    /// For example, the configured `server_root_url` host --> itself.
    ...
    fn include_warp_http_headers<U: IntoUrl + Clone>(url: U) -> bool {
        url.into_url().is_ok_and(|url| {
            url.host_str().is_some_and(|dest_host| {
                ...
                // Same-origin check against the dynamic server host (e.g. app.warp.dev,
                // staging.warp.dev, or a custom override). The window hostname should match.
